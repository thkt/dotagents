...
6. 公開したIssueから今回の決定、合意範囲と根拠、未解決事項、次の判断を辿れることを確認する。[調査成果の引き継ぎ](session.md#調査成果の引き継ぎ)に従い、共有する報告を含むcommit・公開先をIssueへ記録する。Issue URLと合意範囲に加え、今回必要な報告の参照・確認済みblob ID、開始commit、保存・commit・公開の状態を伝える。必要な報告がなければその判断を明示し、不要な報告は作らない。揃わないものは不足として示す。商品実装や実装CLIの起動はこの呼び出しに含めない。
