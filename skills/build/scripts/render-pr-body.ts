...
export const PROTOCOL = 'codex-build-pr-body/v1';
export const DESCRIPTION_PROTOCOL = 'codex-build-pr-body-description/v1';
const REQUIRED_KEYS = ['issue', 'tests_pass', 'gates_pass'];

const LABELS = {
  english: {
    header: "_Automated build verification. It checks the diff against the issue plan; it is not a deep code review._",
  ...
  japanese: {
    header: '_build の自動検証結果。issue plan と diff の機械的な突合であり、深いコードレビューではない。_',
...
function renderSection<T>(label: string, items: readonly T[], renderItem: (item: T) => string): string | null {
...
export function render(payload: unknown): string {
  ...
  const language = typeof payload.language === 'string' ? payload.language.toLowerCase() : 'english';
...
export function main(argv: readonly string[] = process.argv.slice(2)) {
  ...
  return { output: render(payload), exitCode: 0 };
}
