...
Standalone Research returns unignored `research/records/<research_id>.json` and its generated `research/reports/<research_id>.md` for human review and commit. Do not edit generated views or stage, commit, push or publish an Issue. Automatic children return private audited evidence only. Deliberate sharing requires a new standalone invocation selecting `retained_child_report`, with its retained completed child state and ownership; the runtime captures the original as dated context and repeats investigation and both audits against a fresh snapshot. See the [corpus contract](../../research/README.md) for canonical identity, interrupted-pair recovery and retained-state incompatibility. Successful Research attempts to rebuild a private topic index of canonical originals only. A Knowledge write failure does not invalidate the Research report. Keep workflow artifacts in English. Translate only the final user-facing report into the configured language, including the answer, finding and unknown counts, artifact paths, and `think` as the next state. Do not continue into Think.
