...
正本: implementation-start.json / Git blob: da7a70961a392e3c811ca65c9196a348feb3690b / 選択ID: preserve-work, start-objects, authority, start-identity, isolation, setup-recheck, less-rework, index-counterexample, revisit-identity

適用範囲: thkt/dotagentsの実装開始条件。#85・PR #89の採用内容を#90の初回対象として表現する。

これは対象repoの知識です。今回の要求・許可はIssueと合意記録、実行制御は信頼するホストが担当します。形式照合は意味・合意・効果を保証しません。関係先のIDは参照であり、未選択の定義を要求へ追加しません。

## preserve-work (teleology / purpose / agreed)
...
## authority (ontology / rule / agreed)
...
## start-identity (nomology / rule / agreed)

開始HEAD、.dotagents.json、選択した必要入力を照合する。必要入力は開始commitの通常ファイルで確認済みblob・checkout本文が一致し、indexやモードを含む未commit差分がないことを要求する。欠落や違いでは停止し、未確認のIDへ差し替えない。報告を指定するCLIでは--start-commitも必要である。
...
## isolation (nomology / rule / agreed)
...
適用条件: 新規development実行。既存runや同名branchは再利用せず、旧記録を変換・削除しない。
...
## setup-recheck (nomology / rule / agreed)
...
## less-rework (teleology / hypothesis / unverified)
...
## index-counterexample (nomology / observation / observed)

採用済み検証定義には、必要報告や設定のindexを変更した後で本文を元へ戻しても停止するケースがある。本文一致だけで開始可能という説明ではこの条件を表せない。これは既存検証条件による確認例であり、新しい実障害の発生報告ではない。
...
関係: index-counterexample → start-identity: 本文・index・モードを含む説明が必要

関係: index-counterexample → start-objects: 本文とindexの区別

関係: index-counterexample → revisit-identity: 結果から前提を見直す

## revisit-identity (nomology / proposal / proposed)
...
- issue-90: [出典](https://github.com/thkt/dotagents/issues/90) / 版: updatedAt 2026-09-17T11:38:02Z（実装依頼で提示された本文） / agreed / 適用: 初回対象、#85の採用済み要求と共有モデルの範囲。#85・PR #89のオンライン本文は今回再取得できていない

- issue-85: [出典](https://github.com/thkt/dotagents/issues/85) / 版: PR #89 採用commit 7c869f80bd95ea01b47e1ffd35f89e2899657bf2 / agreed / 適用: 開始条件の合意への参照。内容はIssue #90の引用と採用コードで照合し、オンライン本文の再取得とは区別する

- pr-89: [出典](https://github.com/thkt/dotagents/pull/89) / 版: 7c869f80bd95ea01b47e1ffd35f89e2899657bf2 / observed / 適用: ローカルGitのmerge履歴と採用差分。PR本文や当時のCI結果を再検証した記録ではない
