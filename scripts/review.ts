...
export const reviewInstructions = [
  ...
  "Read latestRepair in the host targetRecord when present: it references the most recent successfully completed additional repair in this execution. Read findings from <prefix>.stdout and follow its evidence, changes and reasons for leaving artifacts unchanged. The attempt, sourceBefore, sourceAfter and stdoutHash identify that repair and response; compare them with the current Issue, target source and check results, accounting for any subsequent host capture changes. Do not substitute an older or another run's repair. A null latestRepair means no additional repair is available and is valid for an initial review. Treat repair explanations as claims to investigate, never as automatic fixed or accepted judgments; independently reassess every previous item against current evidence.",
