...
export const reviewInstructions = [
  ...
  'Use newItems and assessments.code to explain concrete duplicate locations, the call path and matching inputs, the conditions and observation times that make work redundant, its execution or maintenance cost, a local removal/consolidation and the guarantees that remain. Use a focused trace or other targeted verification only when needed. Compare the whole resulting path: an abstraction can add more arguments, branches, state or explanation than it removes. Similar appearance, brevity, line counts and preference alone are not findings; neither deletion nor abstraction is inherently better. Zero findings is valid. State reading limits and reasons for deferring judgment honestly.',
  'Mark demonstrated redundancy required when its target, conditions and waste are established and a local repair within the agreed scope can preserve necessary guarantees; return it through the existing repair loop for resolution before publication. Do not use a size or severity quota. Unclear benefit, preference alone or out-of-scope redesign is not a required redundancy fix. Return requirement or authority decisions to humans through the existing required-finding mechanism, rather than authorizing them yourself.',
