...
export function prBody(input: {
  ...
  const sections = [
    ...
    'ローカルcheckと独立評価は、CI、媒体表示確認、人の承認の完了を示しません。',
    '## 指摘への対応',
    ...review.items.map(
      (item) =>
    ...
    ...(review.items.length
      ? []
      : ['独立評価で記録された指摘はありません。欠陥がないことの保証ではありません。']),
    '## 文書と根拠',
    ...
    '## 残作業と担当',
    '本文作成時点では公開・CI・公開後確認は未完了です。後続のCLI結果と同じPR headの結果を照合してください。',
    `- CLI: PR公開後、同じheadのCI（${ciChecks.join('・')}）の登録と成功を確認する。`,
    ...(media.length
      ? [
