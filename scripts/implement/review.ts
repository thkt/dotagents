...
export const reviewInstructions = [
  ...
  'Read the target test policy and independently inspect affected tests against requirements, not implementation claims. In assessments.tests, explain realistic bugs prevented, assurance beyond existing verification, reasons for adding, retaining, consolidating or removing tests, lost detection conditions, remaining verification and unverified limits. Weigh that assurance against runtime, flakiness and maintenance; detect copied expectations and negative tests passing for the wrong reason. Return concrete missed bugs or unjustified costs as ordinary findings for repair, but do not reject justified deletion for lower counts, demand tests where existing verification suffices or require a per-test ledger. For test cleanup, compare implementation, tests, documentation and runtime under the same conditions; file moves or compressed lines alone are not benefits. Distinguish measured results from hypotheses. This assessment reaches the PR description; schema validity does not establish judgment quality or model adherence.',
  ...
  'Read repairsSinceReview in the host targetRecord in order: it references all successfully completed additional repairs in this execution since the previous independent review (or since execution began for the initial review), including repairs followed by failed checks. latestRepair still identifies the most recent entry. Read findings from each <prefix>.stdout and follow its evidence, changes and reasons for leaving artifacts unchanged. The attempt, sourceBefore, sourceAfter and stdoutHash identify that repair and response; compare them with the current Issue, target source and check results, accounting for any subsequent host capture changes. Do not substitute unreferenced repairs from an earlier review interval or another run. An empty repairsSinceReview and null latestRepair mean no additional repair is available and are valid for an initial review. Treat repair explanations as claims to investigate, never as automatic fixed or accepted judgments; independently reassess every previous item against current evidence.',
