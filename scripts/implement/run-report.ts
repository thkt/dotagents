...
function eventDetail(
  ...
  const source =
    metadata.line === undefined
      ? ''
      : `<span class="event-source"><span aria-hidden="true">L${metadata.line}</span><span class="visually-hidden">原記録 events.jsonl の${metadata.line}行目</span></span>`;
...
function inputFiles(command: string) {
  ...
  return {
    ...
    body: `<section class="command-paths"><h4>入力ファイル候補 · 保存commandのパス</h4>${paths.length ? `<ul>${paths.map((path) => `<li>${quote(path)}</li>`).join('')}</ul>` : '<p>抽出できず。元のcommandと原記録を確認してください。</p>'}</section>`,
  };
}
...
function publicationSection(result: RunResult) {
  return `<dl class="summary-fields">${field('公開状態', shown(result.publication))}${field('commit', result.commit ? quote(result.commit) : '未記録')}${field('PR', externalLink(result.url, result.url ?? ''))}${field('CI', shown(result.ci))}</dl>${result.ciDetails === undefined ? '' : detail('CIの保存結果', `<pre>${excerpt(JSON.stringify(result.ciDetails, null, 2), 3000)}</pre>`)}<p class="report-note">GitHubの現在状態ではなく、run終了時の保存記録です。</p>`;
}
...
function activitySection(
  ...
  return `<p class="muted">ホストの保存イベントを順に示します。モデル内の操作は各イベントログ内の順序のみを示し、別ログ間の全体順序は推定しません。</p><aside class="report-notes" aria-labelledby="report-notes-title"><h3 id="report-notes-title">行動ログの読み方</h3><p>L番号は各イベント原記録（events.jsonl）の物理行位置です。ソースコードの行番号ではありません。原記録は各ログの「イベント原記録を開く」から確認できます。</p><p>入力ファイル候補は保存commandから静的に抽出した候補です。実際の読み込み成功やモデルの理解は未確認です。未対応の構文や標準入力などは抽出できず、入力ファイルがないとは限りません。相対パスは解決していません。</p></aside><h3>ホスト工程</h3><div class="actor-group"><div class="activity-list">${hostEvents.join('') || '<p class="empty-list">検証イベントは未記録です。</p>'}</div></div><h3 style="margin-top:24px">実装担当の要約</h3>${workSummary}<h3 style="margin-top:24px">モデルのツール実行・イベント</h3>${actors.join('') || '<p>表示対象のモデルイベントは未記録です。</p>'}`;
}
...
.event-source{display:inline-block;margin-left:8px;white-space:nowrap;font-size:12px;font-weight:400;color:#687484}.visually-hidden{position:absolute;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip-path:inset(50%);white-space:nowrap;border:0}.report-notes{margin:16px 0;padding:14px 16px;border:1px solid #dce1e7;border-radius:8px;background:#f6f7f9;font-size:13px;color:#566170}.report-notes h3{font-size:14px}.summary-fields{display:block}.summary-fields>div{padding:12px 0}.summary-fields>div:first-child{padding-top:0}.summary-fields>div:last-child{padding-bottom:0}.summary-fields>div+div,.result-note{border-top:1px solid #e2e6ec}.result-note{margin-top:18px;padding-top:18px}.review-summary{white-space:pre-wrap}
</style></head><body><main><header><div class="eyebrow">IMPLEMENT / 1 RUN</div><h1>実装runの記録</h1><span class="pill ${escape(result.status)}">${escape(status)}</span></header>
<section class="section card"><h2>今回の結果</h2><dl class="summary-fields">${field('Issue', externalLink(result.issue, result.issue))}${field('対象repo', shown(result.repository))}${field('開始commit', quote(result.startCommit))}${field('開始日時 · UTC', shown(result.startedAt))}${field('終了日時 · UTC', shown(result.finishedAt))}${field('HTML生成日時 · UTC', shown(generatedAt))}${field('終了状態', escape(status))}${field('最終工程', `${shown(result.phase)} / ${shown(result.operation)}`)}</dl><div class="result-note"><h3>停止理由・結果</h3><p>${shown(resultReasons.get(result.reason) ?? result.reason)}</p></div><div class="result-note"><h3>次の対応</h3><p>${shown(nextActions.get(result.nextAction) ?? result.nextAction)}</p><p class="report-note">残る作業: ${result.remaining.length ? result.remaining.map((task) => escape(remainingTasks.get(task) ?? task)).join('、') : '記録なし'}。人の承認・マージは、このrunの結果に含みません。</p></div></section>
