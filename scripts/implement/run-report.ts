...
async function hostEventRows(
  ...
  return Promise.all(
    events.map(async (event, index) => {
      ...
      return eventDetail(
        `ホスト · ${hostRoles.get(event.role) ?? event.role}`,
        status.label,
...
function publicationSection(result: RunResult) {
  return `<dl class="summary-fields">${field('公開状態', badge(result.publication, result.publication === 'published' ? 'info' : result.publication === 'unconfirmed' ? 'pending' : 'neutral'))}${field('commit', result.commit ? quote(result.commit) : '未記録')}${field('PR', externalLink(result.url, result.url ?? ''))}${field('CI', badge(result.ci, ciTones.get(result.ci ?? '') ?? 'neutral'))}</dl>${result.ciDetails === undefined ? '' : detail('CIの保存結果', `<pre>${excerpt(JSON.stringify(result.ciDetails, null, 2), 3000)}</pre>`)}<p class="report-note">GitHubの現在状態ではなく、run終了時の保存記録です。</p>`;
}
...
<div class="grid section"><section class="card"><h2>検証と評価</h2><dl class="summary-fields">${field('ホストcheck', state ? `${state.checks}回記録` : '未記録')}${field('独立評価', badge(review?.status, review ? (review.status === 'accepted' ? 'success' : 'pending') : 'neutral'))}${field('検証終端', state ? shown(state.result ?? undefined) : '未記録')}${field('評価指摘', review ? `${review.items.length}件` : '未記録')}</dl><p class="report-note">acceptedは公開後確認や人の承認を意味しません。</p></section><section class="card"><h2>公開の記録</h2>${publicationSection(result)}</section></div>
