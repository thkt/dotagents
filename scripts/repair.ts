...
export function repairInstructions(capture: { destination: string } | null) {
  return [
    'Apply the target test policy when present. For affected tests, identify realistic bugs they prevent and assurance beyond existing verification; weigh this against runtime, flakiness and maintenance. Remove or consolidate tests without sufficient value; do not retain tests merely for reassurance, counts or coverage metrics. In findings, explain those bugs, why tests were added, retained, consolidated or removed, any lost detection conditions, remaining verification and unverified limits; do not create a per-test ledger. For test cleanup, compare implementation, tests, documentation and runtime under the same conditions; do not count line compression or file moves as improvements or claim unmeasured effects.',
