...
冒頭に「レビュー試験の行動ログと評価」、表示サンプル／実モデル／不明の区分、評価対象、試験の問い、記録された結論と理由、実行開始日時・ホスト判断日時を示します。実行終了日時とHTML生成日時は記録詳細にあります。品質に関わる不足・未達・実行失敗は「残る確認」に、使用量の欠落は「時間・使用量」に分けます。評価と行動ログの一覧は折り畳みを開かずに読めます。期待値と実結果は入力ラベルと保存値で比較でき、指摘一覧では影響・箇所・真偽の裁定を確認できます。長い条件・証拠・対応案・修正状況、原文・原JSON・補足は詳細から辿れます。表示側で文章を要約したり、再判定したりしません。

行動ログは、ホストの制御呼出しを`state.events`の保存順、モデル側の操作を各`events.jsonl`の非空行順で表示します。コマンド・MCPツールの入力や明示された対象、応答、終了コード・状態、記録された時刻を短い行へ整理し、長い値は先頭部分と全文への導線を示します。各行の折り畳みで対応する原文を読めます。スレッド・ターン、モデルの応答なども記録の範囲で示し、未対応イベント・不正行は未確認として残します。別ログ・主体間の全体順序、親子関係や制御呼出しとの対応は推定しません。

開始・完了は同じJSONL内に開始1件・完了1件があり、IDと項目種別が一致し、完了が後の行にある場合だけ相互リンクします。行をまとめて消さず、途中の失敗・中断・更新を保持します。ID欠落・重複・不一致や完了の欠落では対応を未確認とします。stateに実行中の呼出しが残る場合も完了扱いにしません。各ログの件数は非空行数であり、ツール操作数や全実行の総量ではありません。応答サイズは文字列として保存された応答だけのUTF-8バイト数です。時刻や結果の欠落を推定で補わず、参照操作から内容の理解・有効利用や空白時間の原因も判断しません。
