...
中断後の確認は[結果と再実行](#結果と再実行)、公開担当の操作は[PRの公開](#prの公開)、PRと人のレビュー・承認は[レビューを助ける説明](../docs/wiki/development-policy.md#レビューを助ける説明)を参照してください。公開の許可と範囲は[implement](../skills/implement/SKILL.md)と[IssueからPR作成](#issueからpr作成)で確認します。共通check内の制御テストは模擬コマンドを使いますが、ここで説明するCLI試行は実モデルを呼びます。
...
コマンド実行の見出しには、保存されたcommandから静的に抽出した「入力ファイル候補」を表示します。閉じた状態でファイル名を読め、同名ファイルには親ディレクトリも表示します。展開すると省略しないパス、元のcommandと出力を確認でき、イベント原記録へ辿れます。相対パスは実行時の作業ディレクトリへ解決せず、command中の表記から引用符・エスケープを取り除いて表示します。候補は実際の読み込み成功やモデルの理解を示しません。終了コードと開始・未確認状態は別に確認してください。

抽出対象は`cat`、数値または`$`による行範囲を`p`で表示する`sed -n`、`head`・`tail`の静的なファイル引数です。引用した空白入りパス、`;`・`&&`・`||`・パイプ・改行で区切った複数コマンド、`sh`・`bash`・`zsh`の`-c`・`-lc`等による引用済みcommandも扱います。対応するオプションは限定しており、検索パターン、`rg --files`などの列挙候補、リダイレクト先を入力ファイル候補に含めません。標準入力の元ファイルも推定しません。変数・コマンド置換、glob、here-document、制御構文などがあるcommandは抽出を見送ります。単語の先頭にある引用・エスケープされていない`=`も、zshのパス展開と区別できないため、シェルの明記がなくても抽出を見送ります。引用・エスケープされた`=sh`などは文字どおりの候補として扱います。CR文字（復帰）を含むcommandも、文字を削除して別のパスとして表示しないよう、抽出を見送ります。「一部のみ抽出」「抽出できず」は入力ファイルがないという意味ではありません。元のcommandと原記録を確認してください。候補の生成のためにファイルの読取りやコマンド、モデルを再実行することはありません。

生成したHTMLはローカルの`report.html`をブラウザーで開きます。macOSでは`open /absolute/path/to/run/report.html`を使えます。担当AIは完了・停止の報告で、実際の絶対パスと開き方を案内します。

HTML生成に失敗した場合も元の`result.json`は変更せず、成功として報告しません。担当AIは残る`result.json`と次の再生成コマンドを案内します。保存済みの新しいrunから再生成するには、既存HTMLを上書きしない別名を指定します。古いrunの変換・再評価や停止runの再開には使いません。
...
```sh
bun scripts/capture/verify.ts /absolute/path/to/target-repo firefox
```
...
レビュー品質の観測が必要な場合に、共通checkと別にホストで次を実行します。改善案の比較には[改善効果の比較方針](../docs/wiki/development-policy.md#改善効果の比較)を適用し、要求充足・実質差分・費用の順に判断します。保存記録で判定方法だけを比較できる条件と再実行が必要な条件も同方針に従い、比較のために常に新しい試行を求めません。既存のCodex認証を使い、ブラウザーやサーバーの起動、GitHub公開は必要ありません。

```sh
bun scripts/implement/review-probe.ts
```
...
旧セッション、lock、保存済み評価、research、生ログ、worktreeは削除・変換しません。旧セッションは参照資料として必要な事実だけ読み、旧CLIの実行や評価の自動移行を新しい通常経路にしません。進行中の旧タスクは内容と所有者を確認し、途中で実行コードを差し替えません。共通登録の切替・復帰は[保全方針](../docs/wiki/development-policy.md#旧資産の保全と登録変更)に従い、人がマージした採用commitを対象にします。
...
撮影では対象commitと確認する操作・状態を対応づけ、入力途中の変化を示す場合はキーイベントで1文字ずつ入力します。説明が必要ならキー表示・字幕・音声を選び、音声にはmacOS標準音声のKyokoを使ってFFmpegで映像と同期し、合成音声と明記します。探索的な画面確認にagent-browserを使うことは任意で、通常の検証・撮影の必須依存ではありません。
