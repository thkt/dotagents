...
冒頭に「レビュー試験の行動ログと評価」、表示サンプル／実モデル／不明の区分、評価対象、実行開始日時・ホスト判断日時を示します。問い・達成基準を一つのパネルにまとめ、その下にホストの結論・実時間・入力トークン・出力トークンのカードと判断理由を置きます。欠落は未計測・未確認とし、子モデルを含む総使用量・金額を推定しません。モデル時間・キャッシュ入力・集計範囲の原記録は「時間・使用量の内訳と原記録」、実行終了日時・HTML生成日時は記録詳細から確認できます。品質に関わる不足・未達・実行失敗は「残る確認」に、費用比較の未確認は指標カードの下に示します。評価と行動ログの一覧は折り畳みを開かずに読めます。期待値と実結果は入力ラベルと保存値で比較でき、指摘一覧では影響・箇所・真偽の裁定を確認できます。長い条件・証拠・対応案・修正状況、原文・原JSON・補足は詳細から辿れます。表示側で文章を要約したり、再判定したりしません。

行動ログは、ホストの制御呼出しを`state.events`の保存順、モデル側の操作を各`events.jsonl`の非空行順で表示します。モデル側は行番号・記録時刻、操作・ツール、入力・対象、応答・サイズ、状態を表の列で比較できます。ホスト側の表は保存順、操作、対象、実時間・記録、状態を示します。長い値は先頭部分と全文への導線を示します。狭い画面では指標カードと表のセルを縦に並べ、各値のラベルを残します。各行の折り畳みで対応する原文を読めます。スレッド・ターン、モデルの応答なども記録の範囲で示し、未対応イベント・不正行は未確認として残します。別ログ・主体間の全体順序、親子関係や制御呼出しとの対応は推定しません。

開始・完了は同じJSONL内に開始1件・完了1件があり、IDと項目種別が一致し、完了が後の行にある場合だけ相互リンクします。行をまとめて消さず、途中の失敗・中断・更新を保持します。ID欠落・重複・不一致や完了の欠落では対応を未確認とします。stateに実行中の呼出しが残る場合も完了扱いにしません。各ログの件数は非空行数であり、ツール操作数や全実行の総量ではありません。応答サイズは文字列として保存された応答だけのUTF-8バイト数（B）を数値で表示します。文字数や全実行の総量ではなく、構造化応答などはサイズ未計測とします。時刻や結果の欠落を推定で補わず、参照操作から内容の理解・有効利用や空白時間の原因も判断しません。
...
関連記録が欠ける、読めない、JSONが不正、対象ID・基準版・要求・ケース対応表の照合ができない場合も、読める結果からHTMLを生成し、冒頭に不足の存在と必要な照合を示し、同じ欄の折り畳みに詳細をまとめます。`state.events`の非zero終了・timeout、JSONLのerror・failed・非zeroの`item.exit_code`も冒頭に示します。それ以外の自由文から失敗や原因を推測しません。JSONLの不正行・未完の行、未対応イベント、完了との対応不足、中断も同じ解析から警告と行表示へ反映し、原文を保持します。原本の改変を防ぐ署名検証や、記録の完全性・真正性の保証は行いません。生成中の元記録の同時更新は避けてください。
...
このコマンドはcheckout外の新しい一時ディレクトリに公開可能な模擬記録と`html/normal.html`・`pending.html`・`empty.html`・`stopped.html`・`unmet.html`・`missed.html`を生成し、実際のパスを返します。モデル・ブラウザー・サーバーは起動しません。`normal`は同一原因の複数指摘を裁定し条件を満たした表示、`pending`は結論・判断日時・裁定・使用量・関連ログの不足、`empty`は指摘なしで条件を満たした表示、`stopped`は実行失敗、`unmet`は誤指摘による条件未達と補足観測の未確認、`missed`はaccepted・指摘なしでも既知欠陥を見落とした表示です。normalには同じIDを持つコマンドとMCPツールの開始・完了、pendingには中断・完了不足・未知イベント・不正行、unmetにはツールの非zero終了を含めます。後続結果で失敗や不足を隠さないことを確認します。由来はすべて表示サンプル、日時・判断・使用量も模擬値で、実モデルの実測ではありません。
...
3. 評価と行動ログの一覧を折り畳みを開かずに読み、normalのコマンド・MCPツールの入力と応答、同じIDの対応行、pendingの中断・未完・未知・不正行を各JSONLと照合します。別ログの順序や因果が補われていないこと、各行の原文へ辿れることを確認します。狭い画面と通常幅で長い日本語・複数指摘・ログを読み、Tab・Enterでページ内リンク、Tab・EnterまたはSpaceで折り畳みを操作します。フォーカスが見え、リンク先へ移動でき、条件・根拠が欠けず読めることを確認します。
