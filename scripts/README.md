...
[評価の目的とレビュー試験](../.codex/DEVELOPMENT.md#評価の目的とレビュー試験)に従い、指示・入力・モデル等の変更や見落としがあり、既存証拠だけでは判断できない場合に、共通checkと別にホストで次を実行します。問い・対象版・期待結果・必須条件・許容差を既存Issue・PRへ記録し、対象・入力・予算・共有範囲の許可を確認してください。表示変更だけのためには実行しません。改善案の比較には[改善効果の比較方針](../.codex/DEVELOPMENT.md#改善効果の比較)を適用し、要求充足・実質差分・費用の順に判断します。保存記録で判定方法だけを比較できる条件と再実行が必要な条件も同方針に従い、比較のために常に新しい試行を求めません。既存のCodex認証を使い、ブラウザーやサーバーの起動、GitHub公開は必要ありません。

```sh
bun scripts/verify-review.ts
```

OSの一時ディレクトリに公開可能な小さなページ分割関数のfixtureを作り、不具合入りと正しい変更をAstra/highで各1回評価します。既存のcorrection・actorを使い、自動修正はせず、対象と指摘を保持します。各試行はreview 1回、修正への引き継ぎ応答1回、モデル時間20分を上限とし、通常フローの設定は変更しません。これはケース単位のレビュー試験であり、ハーネス全体の採点ではありません。見落とし・誤指摘・未裁定も保存し、CLIの成功と試験の条件充足を区別します。
...
新規の`result.json`は同じケースの`trial`に由来、実行開始・終了日時（UTC）、問い、適用基準、ホスト判断を保存します。`live_model`は合成コードを実モデルでレビューする試験、`display_sample`はモデルを呼ばない表示サンプルです。実行直後の`judgment`の結論・判断日時・理由は`null`で、ホスト裁定は未確認です。問い・基準はモデル入力へ渡しません。

ホストは元の記録を保全した上で、同じケースの`adjudication.findings`に指摘IDごとの`demonstrated`（根拠あり）／`false_positive`（誤指摘）／`unconfirmed`（未確認）、再現と理由を記録し、`missedKnownDefect`を確認します。必要な指摘の真偽を再現と理由で確定し、既知欠陥の見落としの確認も済んだら、`adjudication.status`を`pending_host_adjudication`から`completed`へ更新します。未裁定や必要な根拠の不足が残る間は`pending_host_adjudication`を維持します。指摘なしの正常例でも、要求を満たす独立確認と誤指摘・見落としの確認を終えてから、空の`findings`、`missedKnownDefect: false`、`status: completed`を記録します。指摘0件やモデルの`accepted`だけで完了にしません。

次に`trial.judgment`の`conclusion`（`met`／`unmet`／`pending`／`execution_failed`）、`at`（判断したUTC日時）、`reason`、`unmet`（未達の理由の配列）、`unconfirmed`（確認待ちの配列）を記録します。`adjudication.status`は裁定作業の完了状態、`trial.judgment`は適用基準に対する試験の結論です。誤指摘が確定し、別の指摘が未裁定なら、`status: pending_host_adjudication`のまま`conclusion: unmet`とし、未達の理由と残る確認を両配列に分けて残します。裁定が完了し使用量だけが不足する場合は`status: completed`とし、品質の結論と費用比較の未確認を分けます。分類の基準は開発方針を使ってください。

`judgment.evidence`には判断に使った同じケースの参照を配列で記録します。`reproduction`はケースの再現、`finding:指摘ID`は再現・理由を持つID別裁定、`target:対象ID`は一致する対象・基準版の保存記録、`log:CASE/verification/review-1.stdout`などはrun相対パスの保存ログです。任意のパスを追加読込みする仕組みではありません。結論の理由で、基準とどの観測を照合したかを説明してください。参照の存在は意味の正しさや品質の合格を保証しません。使用量だけが足りない場合、品質の結論と分けて費用比較の未確認を残します。
...
2026-09-20の正常例・欠陥例各1回では、指摘なしと、1件の機能不具合およびそのテスト不足の2指摘を確認しました。当時の観測であり、新基準による現在版の合格ではありません。正誤ラベルを分離した2ケースの実測と裁定は[Issue #138の検証記録](../docs/evidence/review-blinding-138.md)を参照してください。
...
冒頭には試験の問い、このケースについて記録された結論と理由、由来・対象・実行開始日時・ホスト判断日時を表示します。実行終了日時とHTML生成日時は記録詳細にあります。品質の判断に関わる不足・未達・実行失敗は「残る確認」に、使用量の欠落は「時間・使用量」に表示します。期待値と実結果は入力ラベルと保存値で比較でき、未知の構造を含む原JSONも詳細に保持します。指摘一覧は記録済みの影響文を見出しにし、真偽の裁定と箇所を表示します。条件・証拠・裁定理由・対応案・修正状況、ログと原記録は折り畳みから読めます。文章からの要約・再判定は行いません。
...
| `host/CASE/result.json` | 由来・実行日時・問い・適用基準・ホストの結論と判断日時・理由・証拠対応、制御の終了理由、モデルのレビュー、指摘ID・修正状況、ホストの裁定、再現入力・期待値・実結果、時間・使用量・限界 |
...
追加ファイルの通常ファイルは、保存されたbase64を復号してUTF-8の本文をエスケープ表示し、JSON原文も残します。一覧・パス・内容・symlinkの保存形式やbase64が不正な場合は、関連記録の警告として生成結果と冒頭に不足を示し、原記録と一覧内の読めるファイルの本文を保持します。正しいbase64でもUTF-8として読めない内容や、改行・タブ・復帰以外のC0制御文字・DELを含む内容は、保存形式の破損と区別し、本文を表示できない旨を記録欄に示します。symlinkは保存された参照先の文字列だけを表示し、参照先の内容は読み込みません。

関連記録が欠ける、読めない、JSONが不正、対象ID・基準版・要求・ケース対応表の照合ができない場合も、読める結果からHTMLを生成し、冒頭に不足の存在と必要な照合を示し、同じ欄の折り畳みに詳細をまとめます。`state.events`の非zero終了・timeout、JSONLのerror・failed・非zeroの`item.exit_code`も冒頭に示します。それ以外の自由文から失敗や原因を推測しません。JSONLの不正行・未完の行は警告と原文を保持します。原本の改変を防ぐ署名検証や、記録の完全性・真正性の保証は行いません。生成中の元記録の同時更新は避けてください。

要求、ケースの再現、指摘、同じ対象IDの保存記録へページ内リンクで進み、詳細は折り畳みから読めます。新規記録の問い・基準とホストの理由、`judgment.evidence`から再現・裁定・対象・ログへ辿れます。対応する旧記録に`trial`がなければ由来・日時・適用基準・結論は不明のまま表示します。ファイル更新時刻・Issue更新日時・HTML生成日時から評価日時を推定せず、旧結果へ現在の基準を適用しません。過去の判断を再検討する際は元記録を保ち、既存のIssue・PRの証拠に今回の基準・判断日時・結果を区別して残します。HTMLは再判定をしません。指摘と裁定はIDで結び、対応する裁定がなければ未確認を表示します。記録された`accepted`を欠陥なし、`needs_changes`を検出成功には変換しません。`fixed`などの修正状況と真偽の裁定も別です。現在のcheckout、パス先のoracle、別試行や再生成品を過去の証拠として読み込みません。対象記録のhashや保存差分だけで元の全成果物を復元できるとは扱いません。
...
このコマンドはcheckout外の新しい一時ディレクトリに公開可能な模擬記録と`html/normal.html`・`pending.html`・`empty.html`・`stopped.html`・`unmet.html`・`missed.html`を生成し、実際のパスを返します。モデル・ブラウザー・サーバーは起動しません。`normal`は同一原因の複数指摘を裁定し条件を満たした表示、`pending`は結論・判断日時・裁定・使用量・関連ログの不足、`empty`は指摘なしで条件を満たした表示、`stopped`は実行失敗、`unmet`は誤指摘による条件未達と補足観測の未確認、`missed`はaccepted・指摘なしでも既知欠陥を見落とした表示です。unmetにはツールの非zero終了も含め、後続結果で失敗を隠さないことを確認します。由来はすべて表示サンプル、日時・判断・使用量も模擬値で、実モデルの実測ではありません。
...
1. 生成した6ファイルをローカルのブラウザーで開きます。同じ保存先の`records/host/case-*/result.json`、`records/case-*/issue.json`、対象・ログと表示を照合し、要求から再現・裁定根拠・同じ試行の記録へ辿れること、対応がない部分は未確認と分かることを確認します。
2. 冒頭でレビュー試験・表示サンプル・実行日時と判断日時の違い・結論・確認待ちを確認します。normalの`[30,40]`と`[]`、未修正でも欠陥検出として条件を満たすこと、実時間1234.5ms・モデル時間900ms・トークン120／20／35を照合します。pendingのfixed／unconfirmed、emptyの指摘なし、unmetの誤指摘と未確認の併存、missedのacceptedと見落とし、stoppedの実行失敗と関連記録不足が隠れないことを確認します。子モデル総量・金額をゼロと表示していないことも確認します。
