...
コマンド実行の見出しには、保存されたcommandから静的に抽出した「入力ファイル候補」を表示します。閉じた状態では候補のタグだけを添え、同名ファイルには親ディレクトリも表示します。候補が抽出できない場合はタグの領域を置きません。展開すると抽出状態（commandから抽出・一部のみ抽出・抽出できず）、省略しないパス、元のcommandと出力を確認でき、イベント原記録へ辿れます。相対パスは実行時の作業ディレクトリへ解決せず、command中の表記から引用符・エスケープを取り除いて表示します。候補は実際の読み込み成功やモデルの理解を示しません。終了コードと開始・未確認状態は別に確認してください。

明示的な絶対パス`/tmp/`・`/private/tmp/`配下の候補タグには、小さな`tmp`ラベルと落ち着いた別背景色を付けます。`/tmp-other`・`/private/tmp-old`などは対象外です。これはcommand中のパス上の場所を示す印で、用途・不要・削除可・実際の読了を意味しません。相対パスや未解決変数から場所を推定せず、symlinkや実ファイルの存在も確認しません。この意味は「行動ログの読み方」にまとめ、各コマンド行には説明を重ねません。

タグは横並びで、幅が足りないとタグ同士が折り返します。タグ内は改行せず、長いタグは横スクロールで確認できます。キーボードではTabでタグへ移動し、左右キーでスクロールします。通常の折り畳みもイベントと同じ右端のchevron（閉じた時は›、展開時は下向き）で示し、左端のマーカーは重ねません。summaryへのキーボードフォーカスと開閉操作を保ち、hover時の下線はタイトルだけに付きます。

操作タイトル横の小さな「L6」などは、モデル側では各`events.jsonl`の物理行位置です。空行や不正な行も数え、ソースコードの行番号とは区別します。ホスト工程の各行は「撮影 L1」「検証 L2」のように操作名とL番号で示し、「ホスト」の接頭辞は付けません。ホスト側では`verification/state.json`の`state.events`内の1始まりの記録順で、JSONの物理行ではありません。読み上げ用のテキストも両者を区別します。L番号自体はリンクではありません。モデル側は各ログの「イベント原記録を開く」、ホスト側は「原記録」の`verification/state.json`と各工程の保存ログから確認します。入力候補の限界、L番号の意味、ホストの記録順と別ログ間の全体順序を推定しない旨は、行動ログ冒頭の独立した共通注記「行動ログの読み方」に一度だけまとめています。

「今回の結果」「検証と評価」「公開の記録」は、各項目に細い枠・角丸・淡い背景・余白を付けた2列グリッドで表示し、狭幅では1列に切り替えます。停止理由・結果と次の対応は境界線で分けます。acceptedの意味、GitHubの現在状態ではない旨、残る作業の注釈は本文より小さい13pxで表示します。acceptedの注釈は「検証と評価」、GitHubの現在状態ではない旨は「公開の記録」の見出し直下、各項目グリッドの前に置きます。保存事実と未確認事項を示し、HTML生成では検証や公開を再実行しない旨の説明はページ末尾に置いています。

イベントと結果のバッジは、状態の文字列を残したまま色分けします。成功・修正済みは緑、失敗は赤、開始・更新・要対応・結果未確認は黄系、単なる記録・応答完了・公開済みの記録は青系、対象外・未知状態などは中立色です。コマンドの成功色は完了イベントの終了コード0、ホスト工程は時間切れでない終了コード0に基づきます。モデルの応答完了をコマンド成功や内容の検証済みとは扱いません。評価指摘は保存された裁定に従い、CIは保存状態を使います。出力文から成功を推定せず、CI取得不能などはコードの失敗と区別して未確認として表示します。

結果欄の「必須チェックはすべて成功し、登録されたチェックに失敗や保留はない」という定型文と、公開したdraftのCI確認後に担当AIが行う次の対応は、保存値が既知の英文と完全一致した場合だけ日本語に変換して表示します。次の対応では、最新の公開本文とIssue・commit・accepted評価・検証結果の照合、必要媒体の実表示確認、対象・本文・証拠・実行主体・権限・同じheadのCIの再確認、`gh pr ready`、人のレビュー前の切替結果の読戻しという条件と順序を保ちます。既知の残作業名も日本語表示にします。未知の文言は原文のまま表示し、文言から成功状態を推定しません。保存された英語は変更せず、「原記録」の`result.json`から確認できます。

抽出対象は`cat`、数値または`$`による行範囲を`p`で表示する`sed -n`、`head`・`tail`の静的なファイル引数です。引用した空白入りパス、`;`・`&&`・`||`・パイプ・改行で区切った複数コマンド、`sh`・`bash`・`zsh`の`-c`・`-lc`等による引用済みcommandも扱います。対応するオプションは限定しており、検索パターン、`rg --files`などの列挙候補、リダイレクト先を入力ファイル候補に含めません。標準入力の元ファイルも推定しません。`tail +2`・`tail +2c`などの旧形式の位置指定があるコマンドも抽出を見送ります。`tail -- +2`や`cat +2`の`+2`はファイル候補として扱います。変数・コマンド置換、glob、here-document、制御構文などがあるcommandは抽出を見送ります。単語の先頭にある引用・エスケープされていない`=`も、zshのパス展開と区別できないため、シェルの明記がなくても抽出を見送ります。引用・エスケープされた`=sh`などは文字どおりの候補として扱います。CR文字（復帰）を含むcommandも、文字を削除して別のパスとして表示しないよう、抽出を見送ります。「一部のみ抽出」「抽出できず」は入力ファイルがないという意味ではありません。元のcommandと原記録を確認してください。候補の生成のためにファイルの読取りやコマンド、モデルを再実行することはありません。
