...
操作タイトル横の小さな「L6」などは、モデル側では各`events.jsonl`の物理行位置です。空行や不正な行も数え、ソースコードの行番号とは区別します。ホスト側では`verification/state.json`の`state.events`内の1始まりの記録順で、JSONの物理行ではありません。読み上げ用のテキストも両者を区別します。L番号自体はリンクではありません。モデル側は各ログの「イベント原記録を開く」、ホスト側は「原記録」の`verification/state.json`と各工程の保存ログから確認します。入力候補の限界、L番号の意味、ホストの記録順と別ログ間の全体順序を推定しない旨は、行動ログ冒頭の独立した共通注記「行動ログの読み方」に一度だけまとめています。

「今回の結果」「検証と評価」「公開の記録」は、各項目に細い枠・角丸・淡い背景・余白を付けた2列グリッドで表示し、狭幅では1列に切り替えます。停止理由・結果と次の対応は境界線で分けます。acceptedの意味、GitHubの現在状態ではない旨、残る作業の注釈は本文より小さい13pxで表示します。保存事実と未確認事項を示し、HTML生成では検証や公開を再実行しない旨の説明はページ末尾に置いています。
