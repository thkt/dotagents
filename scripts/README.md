...
`judgment.evidence`には判断に使った同じケースの参照を配列で記録します。`reproduction`はケースの再現、`finding:指摘ID`は再現・理由を持つID別裁定、`target:対象ID`は一致する対象・基準版の保存記録、`log:CASE/verification/review-1.stdout`などはrun相対パスの保存ログです。任意のパスを追加読込みする仕組みではありません。結論の理由で、基準とどの観測を照合したかを説明してください。ログ全体の参照しかない場合、個別行動との対応は未確認です。参照の存在は意味の正しさや品質の合格を保証しません。使用量だけが足りない場合、品質の結論と分けて費用比較の未確認を残します。
...
冒頭に「レビュー試験の行動ログと評価」、表示サンプル／実モデル／不明の区分、評価対象、実行開始日時・ホスト判断日時を示します。「評価の要約」にはホストの結論・実時間・入力トークン・出力トークンの4枚のカード、残る確認、問い・達成基準と判断理由を置きます。欠落は未計測・未確認とし、子モデルを含む総使用量・金額を推定しません。モデル時間・キャッシュ入力・集計範囲は使用量の詳細、実行終了日時・HTML生成日時は記録詳細から確認できます。品質に関わる不足・未達・実行失敗と、費用比較の未確認は区別します。評価と行動ログの一覧は折り畳みを開かずに読めます。対象コードの動作確認、指摘の条件・根拠、呼出しの入出力、原記録は共通の折り畳みを使い、すべて初期状態は閉じています。指摘ではモデルの主張・ホストの確認・修正状況を分けます。表示側で合否や原因を再判定しません。

「根拠と再現」の「試験に仕込んだ欠陥」は試験用コードの欠陥を示し、ホストが記録した見落としの有無・確認待ちと分けて表示します。ハーネス自身の欠陥や表示側の検出判定ではありません。ケース再現の全項目は記録詳細の「評価・実行条件の原記録」へまとめ、動作確認欄からリンクします。この全項目表示も解析した要約であり、正確な字句と裁定の原データは同じグループの`result.json`から確認します。モデルのレビュー本文・評価観点・申告した参照文書・引き継ぎは「モデルのレビュー記録」に置き、指摘一覧の後ろには重ねません。要求、呼出しの入出力、評価・実行条件、レビュー対象はカード内で分け、原記録を共通の折り畳みから読めます。

行動ログは、ホストの制御呼出しを`state.events`の保存順、モデル側の操作を各`events.jsonl`の非空行順で表示します。閉じた操作行に見出し・状態・失敗や不足を残し、開くと入力を青、出力を紫と文言で区別して示します。ホストの対象・実時間・関連記録、モデルの保存された入出力・行番号・時刻・原文へ辿れます。狭い画面ではカードと入出力を縦に並べます。スレッド・ターン、モデルの応答なども記録の範囲で示し、未対応イベント・不正行は未確認として残します。別ログ・主体間の全体順序、親子関係や制御呼出しとの対応は推定しません。

JSONの原文表示は、文字列外の空白だけを整形します。巨大整数・指数表記・重複キー・キー順序・文字列の空白とエスケープを保ち、非JSONや空白だけの記録を推測で修復しません。操作の入力・出力も元JSONから一意に特定できるフィールドの字句を取り出します。対象フィールドが重複する場合は選ばず抽出不能を示し、イベント全体の原文を残します。MCPの出力欄は`result`と`error`を分け、`null`も表示します。どちらかに一意に抽出できる非`null`値があれば応答記録ありとし、両方が欠落・`null`・抽出不能の場合は応答欠落を示します。操作名・状態・評価値の解析した要約は、数値の丸めや重複キーの扱いを含め原文と同じではありません。正確な字句は対応する原記録で確認してください。記録の忠実な表示は、全コンテキストの収集完了を意味しません。

「ログから確認した参照資料」は、MCPの明示的な`read_source`・`read_file`・`read_text_file`の`path`／`file_path`と、`read_url`・`fetch_url`・`web_fetch`のHTTP(S) URLを抽出します。パス指定の不一致・重複などで対象が曖昧な場合は一覧へ補完せず、行動ログの原文を残します。対象・提供元・ツールと元のイベント行を結び、開始・更新・完了・失敗・中断・応答欠落を行ごとに表示します。シェル経由の読取り、検索結果、モデルが申告したREADME等は抽出対象外です。ログにない初期コンテキスト、保存応答の全文性、モデルの読了・理解は確認できません。

開始・完了は同じJSONL内に開始1件・完了1件があり、IDと項目種別が一致し、完了が後の行にある場合だけ相互リンクします。行をまとめて消さず、途中の失敗・中断・更新を保持します。ID欠落・重複・不一致や完了の欠落では対応を未確認とします。stateに実行中の呼出しが残る場合も完了扱いにしません。各ログの件数は非空行数であり、ツール操作数や全実行の総量ではありません。応答サイズは文字列として保存された応答だけのUTF-8バイト数（B）を数値で表示します。文字数や全実行の総量ではなく、構造化応答などはサイズ未計測とします。MCPは計測対象のフィールド名も示し、`result`と`error`の合計とは扱いません。時刻や結果の欠落を推定で補わず、参照操作から内容の理解・有効利用や空白時間の原因も判断しません。
...
関連記録が欠ける、読めない、JSONが不正、対象ID・基準版・要求・ケース対応表の照合ができない場合も、読める結果からHTMLを生成し、冒頭に不足の存在と必要な照合を示し、同じ欄の折り畳みに詳細をまとめます。`state.events`の非zero終了・timeout、JSONLのerror・failed・非zeroの`item.exit_code`、MCPの`error`や`result.isError: true`も冒頭に示します。完了したコマンド・MCPの応答欠落も未確認とします。それ以外の自由文から失敗や原因を推測しません。JSONLの不正行・未完の行、未対応イベント、完了との対応不足、中断も同じ解析から警告と行表示へ反映し、原文を保持します。関連JSONが不正でも、読み取れた原文を警告とともに残します。原本の改変を防ぐ署名検証や、記録の完全性・真正性の保証は行いません。生成中の元記録の同時更新は避けてください。
...
見た目には版固定の`basecoat-css@1.0.2`を使い、CSSを生成HTMLへ埋め込みます。閲覧時のCDN・JavaScript・常駐サーバーは不要です。配置・日本語・入出力の色・折り畳みは[専用CSS](review-report.css)で整えます。文章は固定済みのBun 1.4.2の[Markdown API](https://bun.com/docs/runtime/markdown)で描画します。見出し、段落、強調、リスト、表、コード、リンクを扱い、raw HTMLは無効にします。描画後のリンクはHTTP(S)とページ内参照だけを有効にし、画像は代替文字列にします。ログ・JSONはエスケープし、スクリプトや外部リソースの読込みもCSPで制限します。Markdown専用ライブラリやReactは追加していません。他ライブラリとの実測比較や性能上の優位は主張しません。
...
このコマンドはcheckout外の新しい一時ディレクトリに公開可能な模擬記録と`html/normal.html`・`pending.html`・`empty.html`・`stopped.html`・`unmet.html`・`missed.html`を生成し、実際のパスを返します。モデル・ブラウザー・サーバーは起動しません。`normal`は同一原因の複数指摘を裁定し条件を満たした表示、`pending`は結論・判断日時・裁定・使用量・関連ログの不足、`empty`は指摘なしで条件を満たした表示、`stopped`は実行失敗、`unmet`は誤指摘による条件未達と補足観測の未確認、`missed`はaccepted・指摘なしでも既知欠陥を見落とした表示です。normalには同じIDを持つコマンドとMCPツールの開始・完了、巨大整数・重複キー・指数表記・エスケープ・HTMLに見える応答、pendingには中断・完了不足・読取り失敗・応答欠落・未知イベント・不正行、unmetにはツールの非zero終了を含めます。後続結果で失敗や不足を隠さないことを確認します。由来はすべて表示サンプル、日時・判断・使用量も模擬値で、実モデルの実測ではありません。
...
3. 評価と行動ログの一覧を折り畳みを開かずに読み、閉じた操作行の状態・不足と、開いたnormalのコマンド・MCPツールの入力と応答、同じIDの対応行、pendingの中断・読取り失敗・応答欠落・未完・未知・不正行を各JSONLと照合します。対象コードの動作確認を含め折り畳みが初期状態で閉じ、入力は青、出力は紫と文言で区別されることも確認します。normalの`boundary.json`の巨大整数・重複キー・指数表記・エスケープが入力・出力・原文に残ること、参照資料がモデル申告でなくイベント行へリンクすることを照合します。別ログの順序や因果が補われていないこと、各行の原文へ辿れることを確認します。狭い画面と通常幅で長い日本語・複数指摘・ログを読み、Tab・Enterでページ内リンク、Tab・EnterまたはSpaceで折り畳みを操作します。フォーカスが見え、リンク先へ移動でき、条件・根拠が欠けず読めることを確認します。
