...
タグは横並びで、幅が足りないとタグ同士が折り返します。タグ内は改行せず、長いタグは横スクロールで確認できます。キーボードではTabでタグへ移動し、左右キーでスクロールします。hover時の下線は操作タイトルに付きます。

操作タイトル横の小さな「L6」などは、各イベント原記録（`events.jsonl`）の物理行位置です。空行や不正な行も行位置に数え、ソースコードの行番号とは区別します。読み上げ用のテキストにも原記録の行位置を含めています。L番号自体はリンクではなく、各ログの「イベント原記録を開く」から原文を確認します。入力候補の静的抽出・未確認事項・相対パス未解決の説明と、L番号の意味は、行動ログ冒頭の独立した共通注記「行動ログの読み方」に一度だけまとめています。

「今回の結果」「検証と評価」「公開の記録」は、項目間の境界線と余白で区切って表示します。acceptedの意味、GitHubの現在状態ではない旨、残る作業の注釈は本文より小さい13pxで表示します。保存事実と未確認事項を示し、HTML生成では検証や公開を再実行しない旨の説明はページ末尾に置いています。
