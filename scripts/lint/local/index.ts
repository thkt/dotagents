...
function copiesReducerAccumulator(
  ...
  if (method.name === 'assign' && isGlobalCopyOwner(sourceCode, method.object, 'Object')) {
  ...
  } else if (method.name === 'from' && isGlobalCopyOwner(sourceCode, method.object, 'Array')) {
    ...
    const arrayAccumulator =
      initialValue !== undefined && isKnownArrayExpression(sourceCode, initialValue);
    copiesAccumulator = arrayAccumulator && isAccumulator(method.object);
  }
  return copiesAccumulator;
}
