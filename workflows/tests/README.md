...
- `shared/`: reusable repository, model, schema, and text behavior.
- `runtime/`: invocation authority, CLI I/O, storage, and artifact naming.
...
- `research/knowledge.test.ts`: derived topic indexes and bounded original-report lookup.
...
| Issue publishes Markdown and one canonical JSON Plan; Build reads that Issue once                                     | `issue/pipeline.test.ts`, `build/smoke.test.ts`                                     |
...
| Completed Research updates Knowledge and Think receives bounded original reports without duplicating selected reports | `research/knowledge.test.ts`, `research/pipeline.test.ts`, `think/pipeline.test.ts` |
| External write approval and hook-bound inputs remain task- and repository-scoped                                      | `runtime/invocation.test.ts`, `execution/controller.test.ts`                        |
