...
export function prepareShipInput(state: FlowState): void {
  ...
  atomicWrite(prInputPath(state.run_id), {
    ...
    // Historical Plan prose remains implementation authority, not observed Build facts.
    outcome: `Build for Issue #${state.build_plan.issue}.`,
    unit_goals: state.build_plan.units.map(
      (unit) => `${unit.id} — declared scope: ${unit.files.join(', ')}.`,
    ),
