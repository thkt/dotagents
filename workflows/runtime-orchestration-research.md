...
P2 の最終 `bun run check` は 276 pass / 0 fail。実プロセスを調査 dispatch 前、候補保存後、監査前、監査保存後、公開状態保存後、JSON 保存後に終了して再開した。修正途中の文脈保持、2回の未受理 dispatch による再試行上限、競合所有、変更された入力・snapshot・dispatch の拒否、削除された live scope の snapshot による検証、保存先設定変更後の部分公開復旧も確認した。旧 Research Report を使う Think / Knowledge の既存テストを維持した。
