...
- Research saves publication paths and a fixed generation time before writing paired artifacts, deriving the Report from its accepted candidate. Completed retries verify exact input and reuse intact artifacts without writes or a snapshot; missing or divergent Markdown is rebuilt. Research rejects incompatible active state formats while retaining them for recovery. Research reports remain the evidence record. Knowledge is a rebuildable index of original reports; it never derives decisions from Issue artifacts.
...
- Cross-stage state is versioned independently. Think and execution state reject incompatible saved runs with recovery guidance; completed Research and Think report formats remain unchanged.
